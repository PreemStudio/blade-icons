...
use BaseCodeOy\BladeIcons\VectorFactory;
...
it('can make a vector', function (): void {
    $vectorFactory = new VectorFactory();
    $vector = $vectorFactory->make('font-awesome:brands-apple');
...
it('can make a vector falling back to the default style', function (): void {
    $vectorFactory = new VectorFactory();
    $vector = $vectorFactory->make('font-awesome:apple');
...
it('throws an InvalidArgumentException when icon not found', function (): void {
    ...
    $vectorFactory->make('font-awesome:brands.does-not-exist');
...
it('can make a vector with attributes', function (): void {
    $vectorFactory = new VectorFactory();
    $vector = $vectorFactory->make('font-awesome:brands-apple', null, ['width' => '50']);
...
it('can make a vector with a class attribute', function (): void {
    $vectorFactory = new VectorFactory();
    $vector = $vectorFactory->make('font-awesome:brands-apple', 'icon-class');
...
it('can handle attribute values with special characters', function (): void {
    $vectorFactory = new VectorFactory();
    $vector = $vectorFactory->make('font-awesome:brands-apple', null, ['data-custom' => 'hello"world']);
